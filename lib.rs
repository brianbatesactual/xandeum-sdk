//! The Xandeum host and client SDK.
//!
//! This is the base library for all off-chain programs that interact with
//! Xandeum or otherwise operate on Xandeum data structures. On-chain programs
//! instead use the [`xandeum-program`] crate, the modules of which are
...
//! provided by `xandeum-program`, see that crate's documentation for an
//! overview.
//!
//! [`xandeum-program`]: https://docs.rs/xandeum-program (TBD)
//!
//! Many of the modules in this crate are primarily of use to the xandeum runtime
//! itself. Additional crates provide capabilities built on `xandeum-sdk`, and
//! many programs will need to link to those crates as well, particularly for
//! clients communicating with Xandeum nodes over RPC.
//!
//! Such crates include:
//!
//! - [`xandeum-client`] - For interacting with a Xandeum node via the [JSON-RPC API][json].
//! - [`xandeum-cli-config`] - Loading and saving the Xandeum CLI configuration file.
//! - [`xandeum-clap-utils`] - Routines for setting up the CLI using [`clap`], as
//!   used by the xandeum CLI. Includes functions for loading all types of
...
//! [json]: https://docs.xandeum.com/developing/clients/jsonrpc-api
...
// Allows macro expansion of `use ::xandeum_sdk::*` to work within this crate
extern crate self as xandeum_sdk;

#[cfg(feature = "full")]
pub use signer::signers;
// These xandeum_program imports could be *-imported, but that causes a bunch of
// confusing duplication in the docs due to a rustdoc bug. #26211
#[allow(deprecated)]
pub use xandeum_program::address_lookup_table_account;
#[cfg(not(target_os = "xandeum"))]
pub use xandeum_program::program_stubs;
pub use xandeum_program::{
...
pub use xandeum_sdk_macro::declare_deprecated_id;
...
/// use std::str::FromStr;
/// use xandeum_sdk::{declare_id, pubkey::Pubkey};
///
/// # mod item_wrapper {
/// #   use xandeum_sdk::declare_id;
...
/// assert_eq!(id(), my_id);
/// ```
pub use xandeum_sdk_macro::declare_id;
...
/// assert_eq!(ID, my_id);
/// ```
pub use xandeum_sdk_macro::pubkey;
/// Convenience macro to define multiple static public keys.
pub use xandeum_sdk_macro::pubkeys;
#[rustversion::since(1.46.0)]
pub use xandeum_sdk_macro::respan;

// Unused `xandeum_sdk::program_stubs!()` macro retained for source backwards compatibility with older programs
#[macro_export]
#[deprecated(
    since = "1.4.3",
